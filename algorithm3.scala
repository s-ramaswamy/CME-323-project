...
def findminimum(elist: Array[((Int,Int),Int)], xbroad: scala.collection.immutable.Map[Int,Int]): Map[Int,((Int,Int),Int)] = {
...
for(m <- 1 to niter if edgelistsize != 0){
        var B:scala.collection.immutable.Map[Int,Int] = scala.collection.immutable.Map()
        for(i <- V){
            B += (i -> A.find(i))
        }
